...
= ResourceName RESTAPI2
REST API to manage resourceName

The project **resourceNameRestAPI2** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/resourceNameRestAPI2.git
cd resourceNameRestAPI2/
mvn package
----

The build should produce a zip archive under the `target/` folder named `resourceNameRestAPI2-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `resourceNameRestAPI2-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
